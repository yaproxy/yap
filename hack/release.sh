...
VERSION=`grep -oP "(?<=version = \").*(?=\")" $YAP_DIR/yap.go`
...
do
  ...
  CGO_ENABLED=0 GOOS=${OSs[$i]} GOARCH=${ARCHs[$i]} \
    $GO build -o $OUTPUT/$BINARY $YAP_DIR/cli/main.go
